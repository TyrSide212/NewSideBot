...
description = official[0][0] == String(contact[0]) ? 'Solo temas de Side-Bot' : official[1][0] == String(contact[0]) ? lenguajeGB.smsContacto4() : official[2][0] == String(contact[0]) ? lenguajeGB.smsContacto4() : official[3][0] == String(contact[0]) ? lenguajeGB.smsContacto4() : desc === '' ? lenguajeGB.smsContacto5() : desc
...
console.log(e)}} 
handler.help = ['Tyr', 'creator']
handler.tags = ['info']
handler.command = /^(Tyr|creator|propietario|dueño|dueña|propietaria|dueño|creadora|creador|contactos?|contacts?)$/i
...
const sections = [
   {
	title: `PROPIETARIO/Tyr`,
	rows: [
	    {title: "📱 • NOMBRE", rowId: ".Tyr nombre"},
	{title: "🙌 • NUMERO", rowId: ".Tyr bio"},
...
╰━━━━━✦ *${vs}* ✦━━━━⬣`,
  ...
  try {
    if (/(contacto|Tyr|creator|propietario|dueño|dueña|propietaria|dueño|creadora|creador)/i.test(command)) {
      const count = args[1] && args[1].length > 0 ? Math.min(99999999, Math.max(parseInt(args[1]), 1)) : !args[1] || args.length < 3 ? 1 : Math.min(1, count)
        switch (type) {
          case 'nombre':
          await conn.reply(m.chat, `𝙉𝙊𝙈𝘽𝙍𝙀 𝘿𝙀𝙇 𝘽𝙊𝙏 : ${gt} 🐈`, fkontak, m)
         // conn.reply(m.chat, "Nombre del bot : Side-Bot-MD 🐈", m, { contextInfo: { mentionedJid: [nowner] }})
            break
            case 'bio':
...
handler.help = ['Tyr', 'creator']
handler.tags = ['info']
handler.command = /^(contacto|Tyr|creator|propietario|dueño|dueña|propietaria|dueño|creadora|creador)$/i
export default handler*/
