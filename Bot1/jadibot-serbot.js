...
Contenido adaptado para Side-Bot-MD por:
...
browser: mcode ? ['Ubuntu', 'Chrome', '110.0.5585.95'] : ['Side-Bot-MD (Sub Bot)', 'Chrome','2.0.0'],
...
console.error(chalk.bold.yellow(`Errore 440 no se pudo enviar mensaje a: +${path.basename(pathGataJadiBot)}`))
}}
...
console.error(chalk.bold.yellow(`Errore 405 no se pudo enviar mensaje a: +${path.basename(pathGataJadiBot)}`))
}
...
*Política de uso:*
github.com/GataNina-Li/Side-Bot-MD/blob/master/terms.md
`}, { quoted: m }) : ''
}}
setInterval(async () => {
if (!sock.user) {
