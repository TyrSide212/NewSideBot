      ...
      ...options
        ...
        sendFile: {
            ...
            async value(jid, path, filename = '', caption = '', quoted, ptt = false, options = {}) {		    
                ...
                //if (fileSize >= 100) throw new Errore('File size is too big!')
                let opt = {}
...
let canalNombre = ["INFINITY-WA 💫", "GB - UPDATE 🐈", "Tips sobre Side-Bot 🤩", "𝗡𝗘𝗪 𝗣𝗥𝗢𝗝𝗘𝗖𝗧 ⟡ YartexBot-MD ✨", "Sky Ultra Plus ☁️", 'Side-Bot Test']
    ...
    async value(jid, text = '', footer = '', buffer, buttons, copy, urls, list, quoted, options) {
        ...
        if (/^https?:\/\//i.test(buffer)) {
            try {
                ...
                console.error("Errore al obtener el tipo MIME:", error)
            }
        } else {
            ...
            try {
               ...
               if (/^image\//i.test(type.mime)) {
                ...
                console.error("Errore al obtener el tipo de archivo:", error);
            }
        }
...
/**
  ...
  async value(jid, text = '', footer = '', messages, quoted, options = {}) {
    try {
      ...
      console.error("Errore en sendCarousel:", error);
      throw error;
    }
  }
},
        ...
        class Button {
          ...
          makeRow(header = "", title = "", description = "", id = "") {
            if (this._currentSelectionIndex === -1 || this._currentSectionIndex === -1) {
              throw new Errore("You need to create a selection and a section first");
            }
          ...
          makeSections(title = "") {
            if (this._currentSelectionIndex === -1) {
              throw new Errore("You need to create a selection first");
            }
            const buttonParams = JSON.parse(this._buttons[this._currentSelectionIndex].buttonParamsJson);
          ...
          setVideo(path, options = {}) {
            if (!path) throw new Errore("URL or buffer needed");
...
export function logic(check, inp, out) {
    if (inp.length !== out.length) throw new Errore('Input and Output must have same length')
