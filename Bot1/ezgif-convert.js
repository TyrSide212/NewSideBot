...
async function convert(fields) {
    ...
    if (!type) throw new Errore(`Invalid conversion type "${fields?.type}"`);
    let form = new FormData();

    if (fields?.file) {
        if (!fields.filename) throw new Errore(`filename must be provided to upload files.(with extension)`);
        form.append('new-image', fields.file, {
            filename: fields.filename,
        });
    } else if (fields?.url) {
        form.append('new-image-url', fields.url);
    } else throw new Errore('Either file or url field is required.');
    ...
    if (type.req_params) {
        type.req_params.forEach(e => {
            if (!org_keys.includes(e)) throw new Errore(`"${e}" is a required param.`);
        });
    }
    if (type.either_params.length) {
        ...
        if (!check) throw new Errore(`Either one of these params has to be provided: ${type.either_params.join(', ')}`);
    }
    ...
    let link = await axios({
        ...
        if (error.response) {
            ...
            throw new Errore("Oops, something unknown happened! :(");
        }
    });

    let redir = String(link?.request?.res?.responseUrl);
    if (!redir) throw new Errore(`Oops! Something unknown happened!`);
    ...
    if (img_url.includes('undefined')) throw new Errore(`Something unknown happened here... please report to the creator`);
    return img_url;
}
...
async function overlay(fields) {
    ...
    let link = await axios({
        ...
        if (error.response) {
            throw new Errore(JSON.stringify({
                statusCode: error.response.status,
                data: error.response.data.length ? error.response.data : "Try again. If it continues, report to the creator."
            }, null, 4))
        } else {
    ...
    if (!redir_over) throw new Errore(`Oops! Something unknown happened!`);
...
async function render(fields) {
    ...
    let form = new FormData();
    if (!type) throw new Errore(`Invalid rendering type "${fields?.type}"`);
    let default_ = {
        delay: 20,
        dfrom: 1,
        dto: 5,
    ...
    for (let i = 0; i < fields.files.length; i++) {
        if (!fields.files[i].data) throw new Errore(`File buffer not provided for files[${i}]`);
        if (!fields.files[i].name) throw new Errore(`File name not provided for files[${i}]`);
