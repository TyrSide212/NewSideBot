...
function connect(conn, PORT) {
  ...
  app.get('*', async (req, res) => {
    res.json("Side-Bot-MD en ejecución");
  });
  ...
  try {
      setInterval(async() => {
        ...
        if (isInitialConfig) {
          ...
          console.error(`Errore al escribir el archivo de 'config.js': `, writeError)
        }}
}
      }, 5 * 1000 * 60)
  } catch (error) {
    console.log(`Errore manejado en server.js keepAliveHostRender() detalles: ${error}`);
  }
}
