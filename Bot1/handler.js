...
const isROwner = [conn.decodeJid(global.conn.user.id), ...global.Tyr.map(([number]) => number)].map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').includes(m.sender)
...
console.error(e)
for (let [jid] of global.Tyr.filter(([number, _, isDeveloper]) => isDeveloper && number)) {
...
let chat = global.db.data.chats[m.chat]
let user = global.db.data.users[m.sender]
if (!['Tyr-unbanchat.js'].includes(name) && chat && chat.isBanned && !isROwner) return // Except this
if (name != 'Tyr-unbanchat.js' && name != 'Tyr-exec.js' && name != 'Tyr-exec2.js' && name != 'tool-delete.js' && chat?.isBanned && !isROwner) return 
...
if (plugin.rowner && plugin.Tyr && !(isROwner || isOwner)) { //número bot Tyr
fail('Tyr', m, this)
continue
}
if (plugin.rowner && !isROwner) { //Tyr
fail('rowner', m, this)
continue
}
if (plugin.Tyr && !isOwner) { //Propietario/Tyr
fail('Tyr', m, this)
continue
}
...
text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'Benvenuto, @user!').replace('@subject', await this.getName(id)).replace('@desc', groupMetadata.desc?.toString() || '😻 𝗦𝘂𝗽𝗲𝗿 𝗚𝗮𝘁𝗮𝗕𝗼𝘁-𝗠𝗗 😻') :
...
"mediaType": 1, 
sourceUrl: 'https://github.com/GataNina-Li/Side-Bot-MD' }}}, { quoted: fkontak2 })
apii.data = ''
...
//let data = global.Tyr.filter(([id, isCreator]) => id && isCreator)
